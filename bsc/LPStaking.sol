...
contract BOGWhitelistPassthrough is IBOGWhitelister {
    ...
    function isWhitelisted(address adr) external view override returns (bool) {
        return migration.isPreExploitHolder(adr);
    }
}

abstract contract BOGWhitelisted is BOGAuth {
    IBOGWhitelister whitelister;
    bool public whitelistEnabled;
    
    constructor (bool _enabled, address _whitelister) {
        whitelistEnabled = _enabled;
        whitelister = IBOGWhitelister(_whitelister);
    }
    
    function isWhitelisted(address adr) public view returns (bool) {
        return whitelister.isWhitelisted(adr);
    }
    
    modifier whitelisted(address adr) {
        require(!whitelistEnabled || isWhitelisted(adr), "!WHITELISTED"); _;
    }
    
    function migrateWhitelister(address _whitelister) external authorized {
        whitelister = IBOGWhitelister(_whitelister);
    }
    
    function setWhitelistEnabled(bool _enabled) external authorized {
        whitelistEnabled = _enabled;
    }
}
...
contract BOGStaking is BOGAuth, BOGPausable, BOGFinalizable, BOGWhitelisted, IBOGStaking {
    ...
    constructor (address _stakingToken, address _rewardToken, bool _whitelisted, address _whitelister, bool _final) BOGAuth(msg.sender) BOGWhitelisted(_whitelisted, _whitelister) {
    ...
    function _stake(address staker, uint256 amount) internal notPaused whitelisted(msg.sender) {
        require(amount > 0);
